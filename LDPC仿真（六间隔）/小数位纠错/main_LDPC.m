...
parfor i=1:6
    ...
    for n =1:run
        ...
        %Alice发射信号并在信道中传播
        [modSignal,noisedsignal,databits,h,g,noiseVar,a_correct_data]=Alice(SNR(i),R);

        %Bob接收信号并解码
        rxbits=Bob(noisedsignal,noiseVar,h,a_correct_data); 
           
        %Eve窃听信号并解码
        rxbits_e=Eve(modSignal,noiseVar,g,a_correct_data);
